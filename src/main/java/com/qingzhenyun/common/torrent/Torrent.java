package com.qingzhenyun.common.torrent;
...
public class Torrent {
    ...
    public Torrent(byte[] torrent, boolean seeder) throws IOException, NoSuchAlgorithmException {
        ...
        this.hex_info_hash = bytesToHex(this.info_hash);
        ...
        log.info("{}-file torrent information:",
                this.isMultifile() ? "Multi" : "Single");
        log.info("  Torrent name: {}", this.name);
        log.info("  Announced at:" + (this.trackers.size() == 0 ? " Seems to be trackerless" : ""));
        ...
        if (this.isMultifile()) {
            log.info("  Found {} file(s) in multi-file torrent structure.",
                    this.files.size());
            int i = 0;
            for (TorrentFile file : this.files) {
                log.debug("    {}. {} ({} byte(s))",
